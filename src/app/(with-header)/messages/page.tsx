...
export default function MessagesPage() {
  ...
  const { conversations, isLoading, error, refresh, addOptimisticConversation, resolveOptimisticConversation } = useConversations();
  ...
  const handleUserSearch = useCallback(async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const data = await apiGet<{ users: IBackendSearchUser[] }>(
        `${ENV.API_BASE_URL}/search/users?keyword=${encodeURIComponent(query.trim())}`
      );
      const users: IConversationParticipant[] = (data.users ?? []).map((u: IBackendSearchUser) => ({
        userId: u.user_id,
        username: u.username,
  ...
  const handleSelectUser = useCallback(
    async (participant: IConversationParticipant) => {
      ...
      // Optimistic: add temp conversation immediately so the list updates before API responds
      ...
      addOptimisticConversation(optimisticConv);
      setIsModalOpen(false);
      router.push(`/messages/${tempId}`);

      try {
        const conv = await messagingService.createConversationAsync(participant);
        resolveOptimisticConversation(tempId, conv);
        // Navigate to real id
        router.replace(`/messages/${conv.id}`);
      } catch {
        resolveOptimisticConversation(tempId, null);
        // Navigate back to inbox on failure
        router.replace('/messages');
      } finally {
        setIsCreating(false);
      }
    },
    [router, conversations, addOptimisticConversation, resolveOptimisticConversation]
  );
