...
import type { IRawSearchUser } from "@/types/search.types";

// ── TYPES — prefixed with I per project convention ────────────────────────────
...
export default function SearchBar({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      searchService.searchUsers(debouncedQuery).catch((): IRawSearchUser[] => []),
    ]).then(([tracks, users]) => {
      if (cancelled) return;
  ...
  const submit = useCallback((value: string) => {
    ...
    if (navigateOnSubmit) router.push(`/search?q=${encodeURIComponent(value.trim())}`);
  }, [onSearch, navigateOnSubmit, router]);

  const pickSuggestion = useCallback((s: ISuggestion) => {
    setQuery(s.label);
    setOpen(false);
      ...
      {open && suggestions.length > 0 && (
        <div style={s.dropdown}>
          ...
          {suggestions.map((sg, i) => (
            ...
            >
              <span style={s.suggIcon}>{sg.type === "track" ? "♪" : "👤"}</span>
...
const s: Record<string, React.CSSProperties> = {
  ...
  dropdownLabel:{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "#444", padding: "10px 14px 6px" },
  suggestion:   { display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "8px 14px", border: "none", cursor: "pointer", textAlign: "left" as const, transition: "background 0.1s" },
  suggIcon:     { fontSize: 13, color: "#ff5500", flexShrink: 0, width: 18 },
  suggBody:     { flex: 1, display: "flex", flexDirection: "column" as const, gap: 1, minWidth: 0 },
  suggLabel:    { color: "#eee", fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const },
  suggSub:      { color: "#555", fontSize: 11 },
  suggType:     { fontSize: 10, color: "#3a3a3a", textTransform: "uppercase" as const, letterSpacing: "0.05em", flexShrink: 0 },
  searchAllBtn: { display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 14px", background: "transparent", border: "none", borderTop: "1px solid #242424", cursor: "pointer", color: "#ff5500", fontSize: 13, fontWeight: 500, textAlign: "left" as const },
};
