...
const USE_REAL_ENV = process.env.USE_REAL_ENV === 'true';

export default defineConfig({
  testDir: './e2e/tests',
  timeout: USE_REAL_ENV ? 60000 : undefined,
  fullyParallel: true,
  forbidOnly: !!process.env.CI,
  retries: process.env.CI || USE_REAL_ENV ? 2 : 0,
  workers: process.env.CI || USE_REAL_ENV ? 1 : undefined,
  reporter: [['html', { open: 'never' }], ['list']],
  use: {
    baseURL: USE_REAL_ENV ? E2E_DEFAULT_DEPLOY_ORIGIN : 'http://localhost:3100',
    trace: 'on-first-retry',
    screenshot: 'only-on-failure',
    video: 'retain-on-failure',
  },
  webServer: USE_REAL_ENV ? undefined : {
    command:
